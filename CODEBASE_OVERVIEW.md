...
| `routes/bookings.ts` | create, `me`, `requests`, `:id`, `:id/accept`, `:id/payment-intent`, `:id/confirm`, `:id/decline`, `:id/cancel`, pickup/return `initiate`/`confirm`, `:id/photos`, `:id/photos/upload`, `:id/zoink-tap` | Router-level auth + verified. `CreateBookingSchema`, `BookingIdParamsSchema`, and the handoff schemas. Photo upload via Multer (10 MB). |
...
| `bookingController.ts` | `createBooking`, `getBooking`, `getMyBookings`, `getIncomingRequests`, `acceptBooking`, `createBookingPaymentIntent`, `confirmBookingPayment`, `declineBooking`, `cancelBooking`, handoff `initiatePickup`/`confirmPickup`/`initiateReturn`/`confirmReturn`, `uploadHandoffPhotos`, `uploadHandoffPhotoImage`, `zoinkTap`. |
...
| `services/bookingsApi.ts` | `/bookings*` — create, list, detail, accept, **`createBookingPaymentIntent` (`POST /:id/payment-intent`)**, **`confirmBookingPayment` (`PATCH /:id/confirm`)**, decline/cancel, handoff initiate/confirm, photo upload, completed photos. Demo → `mockWeek6`. |
